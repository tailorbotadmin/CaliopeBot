...
def extract_rules():
    ...
    print(f"Inicializando cliente Gemini (AI Studio)...")
    genai.configure(api_key=os.environ.get("GEMINI_API_KEY", "AIzaSyALreZ6PZlC3ogJQOLd51ntRSJ8Miwj5Hs"))
    ...
    for docx_path in docx_files:
        ...
        try:
            ...
            # 2. Subir el PDF via File API (Gemini SÍ soporta PDF)
            print(f"  → Subiendo PDF a Gemini File API...")
            pdf_part = genai.upload_file(pdf_file)
            print(f"    PDF subido: {pdf_part.name}")
...
[
  {{
    ...
    "category": "style" | "grammar" | "format" | "typography"
            ...
            # 4. Llamar a Gemini
            print(f"  → Llamando a Gemini 2.5 Flash para inferencia...")
